    ...
    try {
      ...
      if (!passwordRegex.test(password)) {
        return res
          .status(400)
          .send(
            "Password should be 8–12 characters long and include at least one special character"
          );
      }
  ...
  async logout(req, res, next) {
    try {
      ...
      return res
        .status(201)
        .send({
          message: `User of id :${userId} of ${token} has successfully logged out`,
        });
    } catch (err) {
      console.log(err);
  ...
  async logoutAllDeivce(req, res, next) {
    try {
      ...
      return res
        .status(201)
        .send({
          message: `User of id :${userId} , logged out from all devices`,
        });
    } catch (err) {
      console.log(err);
      throw new applicationError("Something went wrong");
